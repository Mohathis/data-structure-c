...
struct topsort{ //to represent verices of topological sort
int n;
struct topsort *next;
};
...
struct topsort *tsort=0; //to represent vertices of topological sort
 ...
 //inserting nodes in topological order list at begining
  ...
  //DFS-VISIT
   void dfs_visit(struct node *u)
   {
    ...
    {
     t1=t->point;
     if(t1->status==0) //if notvisited
     	dfs_visit(t1);
     t=t->next;
    }
    
    topsorting(u->data); //tore the node topological sort list
    return;
   }
